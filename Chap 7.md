...
**Hérédité :**
Par hypothèse de récurence, $\exist P_{opt}$ qui rencontre $[l_1;r_1],...,[l_n;r_n]$ et qui contient $\{p_s|s\in[\![1;i]\!]\}$.

On numérote les éléments de $P_{opt}$ : $popt_1,...,popt_{\rvert P_{opt}\lvert}$, si $popt_{i+1}\not ={p_{i+1}}$, on utilise le même raisonnement que ci-dessous pour montrer que tous les intervalles pour lequels $popt_{i+1}$ est nécéssaire sont rencontrés par $p+1$.
...
On peut faire mieux en complexité spaciale : $\mathcal O (p)$
